...
std::function<double(System)> Lambda::ECI(const HF::OptionsRestricted& rhfopt, const CI::OptionsRestricted& rciopt, Matrix D) {
    return [rhfopt, rciopt, D](System system) {
        ...
        return rhfres.E + CI({rhfres, rciopt.excits}).rci(system, Hms, Jms, false).Ecorr;
    };
}
...
std::function<std::tuple<double, Matrix>(System&)> Lambda::EGCI(const HF::OptionsRestricted& rhfopt, const CI::OptionsRestricted& rciopt, const std::vector<double>& gopt, Matrix D) {
    return [rhfopt, rciopt, gopt, D](System& system) {
        ...
        Matrix G = Gradient({gopt.at(1)}).get(system, Lambda::ECI(rhfopt, rciopt, rhfres.D), false);
        Matrix Hms = Transform::OneelecSpin(system.ints.T + system.ints.V, rhfres.C);
        Tensor<4> Jms = Transform::CoulombSpin(system.ints.J, rhfres.C);
        double Ecorr = CI({rhfres, rciopt.excits}).rci(system, Hms, Jms, false).Ecorr;
